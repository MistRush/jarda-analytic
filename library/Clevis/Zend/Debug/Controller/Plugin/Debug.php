 ...
 *
 * @category   ZFDebug
 * @package    Clevis_Zend_Debug_Controller
 * @subpackage Plugins
...
/**
 * @see Clevis_Zend_Debug_Controller_Plugin_Debug_Plugin_Text
 */
require_once 'Clevis/Zend/Debug/Controller/Plugin/Debug/Plugin/Text.php';
 ...
 * @license    http://code.google.com/p/zfdebug/wiki/License     New BSD License
 */
class Clevis_Zend_Debug_Controller_Plugin_Debug extends Zend_Controller_Plugin_Abstract
{
    /**
     ...
     * @var array
     */
    public static $standardPlugins = array('Cache', 'Html', 'Database', 'Doctrine', 'Exception', 'File', 'Memory', 'Registry', 'Time', 'Variables');
    ...
    public function __construct($options = null)
    {
         ...
         */
        $version = new Clevis_Zend_Debug_Controller_Plugin_Debug_Plugin_Text();
     ...
     *
     * @param array $options
     * @return Clevis_Zend_Debug_Controller_Plugin_Debug
     */
     ...
     *
     * @param Clevis_Zend_Debug_Controller_Plugin_Debug_Plugin_Interface
     * @return Clevis_Zend_Debug_Controller_Plugin_Debug
     */
    public function registerPlugin(Clevis_Zend_Debug_Controller_Plugin_Debug_Plugin_Interface $plugin)
    {
     ...
     * @return Clevis_Zend_Debug_Controller_Plugin_Debug_Plugin_Interface
     */
    ...
    {
        if ( $this->getRequest()->isXmlHttpRequest() && $this->getRequest()->getHeader("sendDebugData") ) {
            $body = array();
            foreach ($this->_plugins as $plugin)
            {
                if ( $plugin instanceof Clevis_Zend_Debug_Controller_Plugin_Debug_Plugin_Dynamic ) {
                    $body[$plugin->getIdentifier()] = $plugin->getData();
                }
            }
            $body['content'] = $this->getResponse()->getBody();
        ...
        // Enable panel debugging
        $html .= '<script type="text/javascript">' . PHP_EOL;
        $html .= 'require(["clevis/debug"], function(Debug) {' . PHP_EOL;
        $html .= '    Debug.enabled = true;' . PHP_EOL;
        $html .= '});' . PHP_EOL;
        $html .= '</script>' . PHP_EOL;
        ...
        foreach ($this->_plugins as $plugin)
        {
            ...
            /* @var $plugin Clevis_Zend_Debug_Controller_Plugin_Debug_Plugin_Interface */
            $html .= '<div id="Clevis_Zend_Debug_' . $plugin->getIdentifier()
                  . '" class="Clevis_Zend_Debug_panel">' . $panel . '</div>';
        }

        $html .= '<div id="Clevis_Zend_Debug_info">';
        ...
        foreach ($this->_plugins as $plugin)
        {
            ...
            /* @var $plugin Clevis_Zend_Debug_Controller_Plugin_Debug_Plugin_Interface */
            $html .= '<span class="Clevis_Zend_Debug_span clickable" onclick="ZFDebugPanel(\'Clevis_Zend_Debug_' . $plugin->getIdentifier() . '\');">';
            ...
            $html .= '<span id="Clevis_Zend_Debug_Tab_' . $plugin->getIdentifier();
            $html .= '">' . $tab . '</span></span>';
        }

        $html .= '<span class="Clevis_Zend_Debug_span Clevis_Zend_Debug_last clickable" id="Clevis_Zend_Debug_toggler" onclick="ZFDebugSlideBar()">&#171;</span>';
    ...
    protected function _loadPlugins()
    {
    	foreach($this->_options['plugins'] as $plugin => $options) {
    	    ...
    	    if (in_array($plugin, Clevis_Zend_Debug_Controller_Plugin_Debug::$standardPlugins)) {
    	        // standard plugin
                $pluginClass = 'Clevis_Zend_Debug_Controller_Plugin_Debug_Plugin_' . $plugin;
    	    } else {
    ...
    protected function _headerOutput() {
        ...
        return ('
            <style type="text/css" media="screen">
                #Clevis_Zend_Debug_debug { font: 11px/1.4em Lucida Grande, Lucida Sans Unicode, sans-serif; position:fixed; bottom:5px; left:5px; color:#000; z-index: ' . $this->_options['z-index'] . ';}
                #Clevis_Zend_Debug_debug ol {margin:10px 0px; padding:0 25px}
                #Clevis_Zend_Debug_debug li {margin:0 0 10px 0;}
                #Clevis_Zend_Debug_debug .clickable {cursor:pointer}
                #Clevis_Zend_Debug_toggler { font-weight:bold; background:#BFBFBF; }
                .Clevis_Zend_Debug_span { border: 1px solid #999; border-right:0px; background:#DFDFDF; padding: 5px 5px; }
                .Clevis_Zend_Debug_last { border: 1px solid #999; }
                .Clevis_Zend_Debug_panel { text-align:left; position:absolute;bottom:21px;width:600px; max-height:400px; overflow:auto; display:none; background:#E8E8E8; padding:5px; border: 1px solid #999; }
                .Clevis_Zend_Debug_panel .pre {font: 11px/1.4em Monaco, Lucida Console, monospace; margin:0 0 0 22px}
                #Clevis_Zend_Debug_exception { border:1px solid #CD0A0A;display: block; }
            </style>
            <script type="text/javascript" charset="utf-8">
                ...
                function ZFDebugCollapsed() {
                    if ('.$collapsed.' == 1) {
                        ZFDebugPanel();
                        jQuery("#Clevis_Zend_Debug_toggler").html("&#187;");
                        return jQuery("#Clevis_Zend_Debug_debug").css("left", "-"+parseInt(jQuery("#Clevis_Zend_Debug_debug").outerWidth()-jQuery("#Clevis_Zend_Debug_toggler").outerWidth()+1)+"px");
                    }
                }
                
                function ZFDebugPanel(name) {
                    jQuery(".Clevis_Zend_Debug_panel").each(function(i){
                ...
                function ZFDebugSlideBar() {
                    if (jQuery("#Clevis_Zend_Debug_debug").position().left > 0) {
                        document.cookie = "ZFDebugCollapsed=1;expires=;path=/";
                        ZFDebugPanel();
                        jQuery("#Clevis_Zend_Debug_toggler").html("&#187;");
                        return jQuery("#Clevis_Zend_Debug_debug").animate({left:"-"+parseInt(jQuery("#Clevis_Zend_Debug_debug").outerWidth()-jQuery("#Clevis_Zend_Debug_toggler").outerWidth()+1)+"px"}, "normal", "swing");
                    } else {
                        document.cookie = "ZFDebugCollapsed=0;expires=;path=/";
                        jQuery("#Clevis_Zend_Debug_toggler").html("&#171;");
                        return jQuery("#Clevis_Zend_Debug_debug").animate({left:"5px"}, "normal", "swing");
                    }
                }
    ...
    protected function _output($html)
    {
        ...
        $body = preg_replace('/(<head.*>)/i', '$1' . $this->_headerOutput(), $body);
        if ( strpos($body, '</body>') != false ) {
            $body = str_ireplace('</body>', '<div id="Clevis_Zend_Debug_debug">'.$html.'</div></body>', $body);
        } else {
            Clevis_Layout::setEnd(str_ireplace('</body>', '<div id="Clevis_Zend_Debug_debug">'.$html.'</div></body>', Clevis_Layout::getEnd()));
        }
