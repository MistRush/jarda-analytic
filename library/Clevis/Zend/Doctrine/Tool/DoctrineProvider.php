 ...
 */
class Clevis_Zend_Doctrine_Tool_DoctrineProvider extends Zend_Tool_Project_Provider_Abstract
    implements Zend_Tool_Framework_Provider_Pretendable
{

    /**
     * @var Clevis_Zend_Doctrine_Registry
     */
    ...
    {
        return array(
            'Clevis_Zend_Doctrine_Tool_Context_DataFixturesDirectory',
            'Clevis_Zend_Doctrine_Tool_Context_MigrationsDirectory',
            'Clevis_Zend_Doctrine_Tool_Context_SqlDirectory',
            'Clevis_Zend_Doctrine_Tool_Context_YamlSchemaDirectory',
        );
    }
    ...
    public function createProject($dsn, $zendProjectStyle = false, $libraryPerModule = false, $singleLibrary = false)
    {
        ...
        $applicationConfigResource->create();
        $applicationConfigResource->addStringItem('pluginpaths.Clevis_Zend_Doctrine_Application_Resource', 'Clevis_Zend_Doctrine/Application/Resource', 'production');
        $applicationConfigResource->create();
        $applicationConfigResource->addStringItem('autoloadernamespaces[]', "Doctrine", 'production');
        $applicationConfigResource->create();
        $applicationConfigResource->addStringItem('autoloadernamespaces[]', "Clevis_Zend_Doctrine", 'production');
        $applicationConfigResource->create();
    ...
    public function createTables()
    {
        ...
        try {
            $models = Clevis_Zend_Doctrine_Core::getLoadedModels();
            $models = Clevis_Zend_Doctrine_Core::filterInvalidModels($models);
    ...
    protected function _createImportSchema()
    {
        ...
        $listener = false;
        $zendStyles = array(Clevis_Zend_Doctrine_Core::MODEL_LOADING_ZEND, Clevis_Zend_Doctrine_Core::MODEL_LOADING_ZEND_SINGLE_LIBRARY, Clevis_Zend_Doctrine_Core::MODEL_LOADING_ZEND_MODULE_LIBRARY);
        if (!in_array($modelLoading, $zendStyles)) {
            $import = new Doctrine_Import_Schema();
        } else {
            $response = $this->_registry->getResponse();
            $listener = new Clevis_Zend_Doctrine_Tool_ResponseImportListener($response);
    ...
    public function generateMigrationFromDbToSchema()
    {
        $this->_initDoctrineResource();
        $this->_loadDoctrineModels();
        $this->_print('Generating miggration class...');

        $migrationsPath = $this->_getMigrationsDirectoryPath();
        $yamlSchemaPath = $this->_getYamlDirectoryPath();
        $yamlSchemaPreviousPath = $yamlSchemaPath . '_previous';
        $migration = new Doctrine_Migration($migrationsPath);
        $numChanges = 0;

        // If generating first version
        if ( !count($migration->getMigrationClasses()) ) {
            $result = Doctrine_Core::generateMigrationsFromModels($migrationsPath);
            if ( $result )
                $numChanges = 1;
            else
        ...
        // Else if generating next version
            ...
            $changes = Doctrine_Core::generateMigrationsFromDiff($migrationsPath,$yamlSchemaPreviousPath,$yamlSchemaPath);
            $numChanges = 0;
        ...
        $d = dir($yamlSchemaPath);
        while (FALSE !== ( $entry = $d->read() )) {
            if ($entry == '.' || $entry == '..') {
                continue;
            }
            $Entry = $yamlSchemaPath . '/' . $entry;
            if (is_dir($Entry)) {
                continue;
            }
            copy($Entry, $yamlSchemaPreviousPath . '/' . $entry);
        }
        $d->close();

        // Print result
        if ( $numChanges > 0 ) {
            $this->_print('Generated migration successfully (' . $numChanges . ' changes).');
        } else {
            $this->_print('No need for migration, no changes made.');
        }
    }

    public function generateMigrationEmpty()
    {
        $this->_initDoctrineResource();
        $migrationsPath = $this->_getMigrationsDirectoryPath();
        
        $this->_print('Generating empty miggration class...');
        Doctrine_Core::generateMigrationClass("migration", $migrationsPath);
        $this->_print('Generated migration successfully.');
    }
    ...
    public function generateMigration($className=null, $fromDatabase=false, $fromModels=false)
    {
        ...
        if ($className) {
            Doctrine_Core::generateMigrationClass($className, $migrationsPath);

            $this->_print('Successfully generated migration class '.$className.'.', array('color' => 'green'));
            $this->_print('Destination Directory: '.$migrationsPath);
    ...
    public function show()
    {
        ...
        $modules = Clevis_Zend_Doctrine_Core::getAllModelDirectories();
    ...
    protected function _loadDoctrineModels()
    {
        ...
        Clevis_Zend_Doctrine_Core::loadModels(
    ...
    protected function _initDoctrineResource()
    {
        ...
        if (!isset($container->doctrine)) {
            throw new Clevis_Zend_Doctrine_DoctrineException('There is no "doctrine" resource enabled in the current project.');
        }
    ...
    /**
     * @return Clevis_Zend_Doctrine_Registry
     */
