 ...
 */
class Clevis_Zend_Doctrine_Import_Schema extends Doctrine_Import_Schema
{
    /**
    ...
    public function importSchema($schema, $format = 'yml', $directory = null, $models = array())
    {
        ...
        if (!in_array($modelLoading, $zendStyles)) {
            throw new Clevis_Zend_Doctrine_DoctrineException(
                "Can't use Clevis_Zend_Doctrine_Schema with Doctrine_Core::ATTR_MODEL_LOADING not equal to 4 (Zend)."
            );
        }
    ...
    protected function _getBuilder()
    {
        if ($this->_builder == null) {
            $this->_builder = new Clevis_Zend_Doctrine_Import_Builder();
        }
    ...
    {
        $this->_modules = Clevis_Zend_Doctrine_Core::getAllModelDirectories();
    }
    ...
    protected function _buildRecord($builder, $definition)
    {
        ...
        if (strpos($className, "Model_") === false) {
            throw Clevis_Zend_Doctrine_DoctrineException::invalidZendModel($className);
        }
        ...
        if (!isset($this->_modules[$moduleName])) {
            throw Clevis_Zend_Doctrine_DoctrineException::unknownModule($moduleName, $className);
        }
    ...
    /**
     * @param Clevis_Zend_Doctrine_Import_ImportListener $listener
     */
    public function setListener(Clevis_Zend_Doctrine_Import_ImportListener $listener)
    {
