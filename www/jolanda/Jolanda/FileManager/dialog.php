...
if(empty($GLOBALS['FileManagerNotUseSession'])){
    if(empty($_SESSION['user']) &&
        empty($_SESSION['app_user']) &&
        empty($_SESSION['kpo_app_user']) &&
        empty($_SESSION['project_app_user']) &&
        empty($_SESSION['kpo_sk_app_user']) &&
        empty($_SESSION['openid']) &&
        empty($_SESSION['cdb_app_logged_user']) &&
...
if (isset($_POST['submit'])) {
    ...
    $available_languages = include __DIR__ . '/lang/languages.php';
...
<head>
    ...
    <link rel="stylesheet" href="<?php echo $config['assets_url']; ?>css/jquery.fileupload.css">
    <link rel="stylesheet" href="<?php echo $config['assets_url']; ?>css/jquery.fileupload-ui.css">
    <!-- CSS adjustments for browsers with JavaScript disabled -->
    <noscript>
        <link rel="stylesheet" href="<?php echo $config['assets_url']; ?>css/jquery.fileupload-noscript.css">
    </noscript>
    <noscript>
        <link rel="stylesheet" href="<?php echo $config['assets_url']; ?>css/jquery.fileupload-ui-noscript.css">
    </noscript>
    ...
    <link href="<?php echo $config['assets_url']; ?>css/style.css?v=<?php echo $version; ?>" rel="stylesheet" type="text/css"/>
    <!--[if lt IE 8]>
    <style>
    ...
    <script src="<?php echo $config['assets_url']; ?>js/plugins.js?v=<?php echo $version; ?>"></script>
    ...
    <script src="<?php echo $config['assets_url']; ?>js/modernizr.custom.js"></script>
    ...
    <script src="<?php echo $config['assets_url']; ?>js/include.js?v=<?php echo $version; ?>"></script>
</head>
<body>
...
<script src="<?php echo $config['assets_url']; ?>js/jquery.iframe-transport.js"></script>
<!-- The basic File Upload plugin -->
<script src="<?php echo $config['assets_url']; ?>js/jquery.fileupload.js"></script>
<!-- The File Upload processing plugin -->
<script src="<?php echo $config['assets_url']; ?>js/jquery.fileupload-process.js"></script>
<!-- The File Upload image preview & resize plugin -->
<script src="<?php echo $config['assets_url']; ?>js/jquery.fileupload-image.js"></script>
<!-- The File Upload audio preview plugin -->
<script src="<?php echo $config['assets_url']; ?>js/jquery.fileupload-audio.js"></script>
<!-- The File Upload video preview plugin -->
<script src="<?php echo $config['assets_url']; ?>js/jquery.fileupload-video.js"></script>
<!-- The File Upload validation plugin -->
<script src="<?php echo $config['assets_url']; ?>js/jquery.fileupload-validate.js"></script>
<!-- The File Upload user interface plugin -->
<script src="<?php echo $config['assets_url']; ?>js/jquery.fileupload-ui.js"></script>
...
<input type="hidden" id="current_url" value="<?php echo str_replace(array(
    '&filter='.$filter,
    '&sort_by='.$sort_by,
            ...
            <div class="tabbable upload-tabbable"> <!-- Only required for left/right tabs -->
                <div class="container1">
                    <ul class="nav nav-tabs">
                        <li class="active">
                            <a href="#baseUpload" data-toggle="tab"><?php echo trans('Upload_base'); ?></a></li>
                    ...
                    {% for (var i=0, file; file=o.files[i]; i++) { %}
                        <tr class="template-download">
                            ...
                            <td>
                                ...
                                {% if (file.error) { %}
                                    <div style="display: none;"><span class="label label-danger" style="display: none;">Error</span> {%=file.error%}</div>
                                {% } %}
                            </td>
                            <td>
...
<div class="container-fluid">
    ...
    function filenameSort($x, $y)
    {
        ...
        if ($x['is_dir'] !== $y['is_dir']) {
            ...
            $result = ($descending) ? $x['file_lcase'] < $y['file_lcase'] : $x['file_lcase'] >= $y['file_lcase'];
            return (int)$result;
        }
    }
                        ...
                        <div class="row-fluid">
                                ...
                                <div id="multiple-selection" style="display:none;">
                                    <?php if ($config['multiple_selection']) { ?>
                                        <?php if ($config['delete_files']) { ?>
                                            <button class="tip btn multiple-delete-btn" title="<?php echo trans('Erase'); ?>" data-confirm="<?php echo trans('Confirm_del'); ?>">
                                                <i class="icon-trash"></i></button>
                                        <?php } ?>
                                        <button class="tip btn multiple-select-btn" title="<?php echo trans('Select_All'); ?>">
                                            <i class="icon-check"></i></button>
                                        <button class="tip btn multiple-deselect-btn" title="<?php echo trans('Deselect_All'); ?>">
                            ...
                            <div class="span2 half view-controller">
                                <button class="btn tip<?php if ($view == 0)
                                    echo " btn-inverse"; ?>" id="view0" data-value="0" title="<?php echo trans('View_boxes'); ?>">
                                    <i class="icon-th <?php if ($view == 0)
                                        echo "icon-white"; ?>"></i></button>
                                <button class="btn tip<?php if ($view == 1)
                                    echo " btn-inverse"; ?>" id="view1" data-value="1" title="<?php echo trans('View_list'); ?>">
                                    <i class="icon-align-justify <?php if ($view == 1)
                                        echo "icon-white"; ?>"></i></button>
                                <button class="btn tip<?php if ($view == 2)
                                    echo " btn-inverse"; ?>" id="view2" data-value="2" title="<?php echo trans('View_columns_list'); ?>">
    ...
    <div class="row-fluid">
        <?php
        $link = "dialog.php?".$get_params.(isset($_GET['basePath']) ? 'basePath='.$_GET['basePath'].'&' : '');
        ?>
        <ul class="breadcrumb">
            ...
            <?php if ($config['show_language_selection']) { ?>
                <li class="pull-right">
                    <a class="btn-small" href="javascript:void('')" id="change_lang_btn"><i class="icon-globe"></i></a>
                </li>
            <?php } ?>
            <li class="pull-right">
                <a id="refresh" class="btn-small" href="dialog.php?<?php echo $get_params.$subdir."&".(isset($_GET['basePath']) ? 'basePath='.$_GET['basePath'].'&' : '').uniqid() ?>"><i class="icon-refresh"></i></a>
            </li>
            ...
            <li>
                <small class="hidden-phone">(<span id="files_number"><?php echo $current_files_number."</span> ".trans('Files')." - <span id='folders_number'>".$current_folders_number."</span> ".trans('Folders'); ?>)</small>
            </li>
            <?php if ($config['show_total_size']) { ?>
                <li>
                    <small class="hidden-phone"><span title="<?php echo trans('total size').$config['MaxSizeTotal']; ?>"><?php echo trans('total size').": ".makeSize($sizeCurrentFolder).(($config['MaxSizeTotal'] !== false && is_int($config['MaxSizeTotal'])) ? '/'.$config['MaxSizeTotal'].' '.trans('MB') : ''); ?></span></small>
                </li>
            <?php } ?>
        </ul>
    </div>
                ...
                foreach ($files as $file_array) {
                    ...
                    <li data-name="<?php echo $file ?>" class="<?php if($file=='..') echo 'back'; else echo 'dir';?> <?php if(!$config['multiple_selection']){ ?>no-selector<?php } ?>" <?php if(($filter!='' && stripos($file,$filter)===false)) echo ' style="display:none;"';?>><?php
                        ...
                        ?><figure data-name="<?php echo $file ?>" data-path="<?php echo $rfm_subfolder.$subdir.$file;?>" class="<?php if($file=="..") echo "back-";?>directory" data-type="<?php if($file!=".."){ echo "dir"; } ?>">
                            ...
                            <?php } ?>
                            <a class="folder-link" href="dialog.php?<?php echo $get_params.rawurlencode($src)."&".($callback?'callback='.$callback."&":'')."&".(isset($_GET['basePath']) ? 'basePath='.$_GET['basePath'].'&' : '').uniqid() ?>">
                                <div class="img-precontainer">
                                    <div class="img-container directory"><span></span>
                                        <img class="directory-img" data-src="<?php echo $config['assets_url'];?>img/<?php echo $config['icon_theme'];?>/folder<?php if($file==".."){ echo "_back"; }?>.png" />
                                    </div>
                                </div>
                        ...
                        <?php } else { ?>
                            ...
                            <div class="box">
                                <h4 class="<?php if ($config['ellipsis_title_after_first_row']) {
                                    ...
                                    <a class="folder-link" data-file="<?php echo $file ?>" href="dialog.php?<?php echo $get_params.rawurlencode($src)."&".(isset($_GET['basePath']) ? 'basePath='.$_GET['basePath'].'&' : '').uniqid() ?>"><?php echo $file; ?></a>
                                </h4>
                            </div>
                            ...
                            <figcaption>
                                <a href="javascript:void('')" class="tip-left edit-button rename-file-paths <?php if ($config['rename_folders'] && !$file_prevent_rename)
                                    echo "rename-folder"; ?>" title="<?php echo trans('Rename') ?>" data-folder="1" data-permissions="<?php echo $file_array['permissions']; ?>">
                                ...
                                <a href="javascript:void('')" class="tip-left erase-button <?php if ($config['delete_folders'] && !$file_prevent_delete)
                                    echo "delete-folder"; ?>" title="<?php echo trans('Erase') ?>" data-confirm="<?php echo trans('Confirm_Folder_del'); ?>">
                        ...
                        <a href="javascript:void('')" class="link" data-file="<?php echo $file; ?>" data-function="<?php echo $apply; ?>">
                            <div class="img-precontainer">
                                ...
                                <div class="img-container">
                                    <img class="<?php echo $show_original ? "original" : "" ?><?php echo $is_icon_thumb ? " icon" : "" ?>" data-src="<?php echo $config['base_url'] . '/' . $src_thumb; ?>">
                                </div>
                            </div>
                            <div class="img-precontainer-mini <?php if ($is_img)
                                ...
                                <div class="filetype <?php echo $file_array['extension'] ?> <?php if (in_array($file_array['extension'], $config['editable_text_file_exts']))
                                    echo 'edit-text-file-allowed' ?> <?php if (!$is_icon_thumb) {
                                    echo "hide";
                                } ?>"><?php echo $file_array['extension'] ?></div>
                                <div class="img-container-mini">
                                    <?php if ($mini_src != "") { ?>
                                        <img class="<?php echo $show_original_mini ? "original" : "" ?><?php echo $is_icon_thumb_mini ? " icon" : "" ?>" data-src="<?php echo $config['base_url'] . '/' . $mini_src; ?>">
                                    <?php } ?>
                                </div>
                            </div>
                            <?php if ($is_icon_thumb) { ?>
                        ...
                        <figcaption>
                            <form action="force_download.php" method="post" class="download-form" id="form<?php echo $nu; ?>">
                                ...
                                <a title="<?php echo trans('Download') ?>" class="tip-right" href="javascript:void('')" <?php if ($config['download_files'])
                                    echo "onclick=\"$('#form".$nu."').submit();\"" ?>><i class="icon-download <?php if (!$config['download_files'])
                                        echo 'icon-white'; ?>"></i></a>

                                <?php if ($is_img && $src_thumb != "") { ?>
                                    <a class="tip-right preview" title="<?php echo trans('Preview') ?>" data-featherlight="<?php echo $config['base_url'] . '/' . $src; ?>" href="#"><i class=" icon-eye-open"></i></a>
                                ...
                                <a href="javascript:void('')" class="tip-left edit-button rename-file-paths <?php if ($config['rename_files'] && !$file_prevent_rename)
                                    echo "rename-file"; ?>" title="<?php echo trans('Rename') ?>" data-folder="0" data-permissions="<?php echo $file_array['permissions']; ?>">
                                ...
                                <a href="javascript:void('')" class="tip-left erase-button <?php if ($config['delete_files'] && !$file_prevent_delete)
                                    echo "delete-file"; ?>" title="<?php echo trans('Erase') ?>" data-confirm="<?php echo trans('Confirm_del'); ?>">
...
<div id="loading_container" style="display:none;">
    ...
    <img id="loading_animation" src="<?php echo $config['assets_url']; ?>img/storing_animation.gif" alt="loading" style="z-index:10001; margin-left:-32px; margin-top:-32px; position:fixed; left:50%; top:50%">
</div>
<!-- loading div end -->
    ...
    <script>
        var tuiTheme = {
            <?php foreach ($config['tui_defaults_config'] as $aopt_key => $aopt_val) {
                if (!empty($aopt_val)) {
                    echo "'$aopt_key':" . json_encode($config['assets_url'] . $aopt_val).",";
                }
            } ?>
        };
    </script>
