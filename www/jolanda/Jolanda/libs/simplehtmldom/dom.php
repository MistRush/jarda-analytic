...
namespace Jolanda\libs\simplehtmldom;
...
class dom {
    ...
    {
        if ($str) {
            // vypneme nacitani z file, nejak to hapruje v block editoru (kdyz byl content moc dlouhy)
            if (preg_match('/^http:\/\//i', $str)/* || (strlen($str) < 500 && is_file($str))*/) {
                $this->load_file($str);
            } else {
