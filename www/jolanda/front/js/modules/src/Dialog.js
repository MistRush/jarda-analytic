class Dialog {
    ...
    render() {
        ...
        if (this.confirmButton && !this._footer.includes(`id="dialog_${this._uid}_confirm_button"`))
            this._footer += `<button class="btn btn-success" id="dialog_${this._uid}_confirm_button" data-dialog-id="dialog_${this._uid}" onclick="${this.confirm_function}">${this._confirmButtonText}</button>`;
        if (this.cancelButton && !this._footer.includes(`id="dialog_${this._uid}_cancel_button"`))
            this._footer += `<button class="btn btn-red" id="dialog_${this._uid}_cancel_button"  data-dialog-id="dialog_${this._uid}" onclick="window['${this._uid}'].close();">${this._cancelButtonText}</button>`;

        let template = `
              <div class="modal-dialog modal-lg modal-dialog-centered" id="dialog_${this._uid}" role="document" style="${customStyle}">
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title">${this._title}</h5>
                    <button type="button" class="close" onclick="window['${this._uid}'].close()" aria-label="Close">
