...
export default class EntityEditor {
    ...
    constructor(entityEditorDef) {
        this.entityEditorDef = entityEditorDef;
        this.editor = document.getElementById(this.entityEditorDef.getId()); //TODO odstranit všude, nebo nahradit az vue ref
        this.form = new Form(this.editor_id);
        this.panel = null;

        // this.dataSaveType = getFromSetting(this.editor, 'data-save-type');
        // if(this.dataSaveType === 'local') {
        //     this.forGrid = getFromSetting(this.editor, 'for-grid');
        //     this.local_tmp_id = getFromSetting(this.editor, 'local-tmp-id');
        // }else{
        //     this.forGrid = null;
        ...
        $(() => {
            ...
            if(this.entityEditorDef.onAfterInit){
                this.entityEditorDef.onAfterInit(this);
            }
        });
    }
    ...
    confirm(closePanel = false, cont = false) {
        ...
        if (this.form.validate()) {
            ...
            Promise.all(filePromises).then((data) => {
                ...
                Helpers.ajax({
                    type: 'POST',
                    url: (this.entity_id == '') ? this.datacreateUrl : this.dataupdateUrl,
                    data: {
    ...
    confirmSuccess(closePanel, data, cont = false) {
        ...
        if (closePanel) {
            if (cont){
                ...
                const backURL = this.entityEditorDef.getBackUrl();
                if(typeof backURL === 'string') {
                    app.router.router.push({
                        name: backURL,
                    });
                } else {
                    app.router.router.push(backURL);
                }

                if(this.forGrid && this.forGrid.editor){
                    this.forGrid.editor.close();
                }
            }
    ...
    //NEW
    get datalistUrl(){
        return this.entityEditorDef.getDatalistUrl();
    }

    get dataupdateUrl(){
        return this.entityEditorDef.getDataupdateUrl();
    }

    get datacreateUrl(){
        return this.entityEditorDef.getDatacreateUrl();
    }

    get entity_id(){
        return this.entityEditorDef.getEntityId();
    }

    set entity_id(value){
        this.entityEditorDef.setEntityId(value);
    }

    get editor_id(){
        return this.entityEditorDef.getId();
    }

    set editor_id(value){
        this.entityEditorDef.id = value;
    }

    get entity_name_column(){
        return this.entityEditorDef.getEntityNameColumn ? this.entityEditorDef.getEntityNameColumn() : undefined;
    }

    set entity_name_column(value){
        return this.entityEditorDef.setEntityNameColumn(value);
    }

    get dataSaveType(){
        return this.entityEditorDef.dataSaveType;
    }

    set dataSaveType(value){
        this.entityEditorDef.dataSaveType = value;
    }

    get forGrid(){
        return this.entityEditorDef.forGrid;
    }

    set forGrid(value){
        this.entityEditorDef.forGrid = value;
    }

    get local_tmp_id(){
        return this.entityEditorDef.local_tmp_id;
    }

    set local_tmp_id(value){
        return this.entityEditorDef.local_tmp_id = value;
    }

    get parent_entity() {
        return this.entityEditorDef.getParentEntities() ?? [];
    }

    set parent_entity(value) {
        this.entityEditorDef.parent_entities = value;
    }

    get subGrids(){
        const subGrids = this.entityEditorDef.getSubGrids();

        return subGrids.map((subGrid) => {
            return window[subGrid.getId()] ?? null;
        });
    }

    get relationSwitches(){
        const relationSwitches = this.entityEditorDef.getRelationSwitches();

        return relationSwitches.map((relationSwitch) => {
            return window[relationSwitch.getId()] ?? null;
        });
    }

    get categoryTrees(){
        const categoryTrees = this.entityEditorDef.getCategoryTrees();

        return categoryTrees.map((categoryTree) => {
            return window[categoryTree.getId()] ?? null;
        });
    }

    get customData(){
        return this.entityEditorDef.customData;
    }

    set customData(value){
        this.entityEditorDef.customData = value;
    }
